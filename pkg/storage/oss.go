...
func NewOss(config OssConfig) (FileSystem, error) {
...
func (r *Oss) ListObjects(ctx context.Context, opt *ListObjectOpts) (*ListObjectRes, error) {
	res := ListObjectRes{
...
func (r *Oss) PresignUpload(ctx context.Context, bucket string, uploadKey string, lifetimeSecs int64) (url string, err error) {
	return "", nil
}

func (r *Oss) AccessToken(ctx context.Context) (cred *Credentials, err error) {

	request := sts.CreateAssumeRoleRequest()
	request.RoleArn = r.config.RoleArn
	request.RoleSessionName = "uploader_"
	request.Scheme = "HTTPS"

	response, err := r.stsClient.AssumeRole(request)
	if err == nil {
