...
func NewS3(config S3Config) (FileSystem, error) {
...
func (r *S3) PresignUpload(ctx context.Context, bucket, uploadKey string, lifetimeSecs int64) (url string, err error) {
	request, err := r.presignClient.PresignPutObject(context.TODO(), &s3.PutObjectInput{
	...
	if err != nil {
		log.Printf("Couldn't get a presigned request to put %v:%v. Here's why: %v\n",
			r.config.Bucket, uploadKey, err)
	}
	return request.URL, err
}

func (r *S3) AccessToken(ctx context.Context) (cred *Credentials, err error) {
	response, err := r.stsClient.AssumeRole(ctx, &sts.AssumeRoleInput{
		DurationSeconds: aws.Int32(3600),
		RoleArn:         aws.String(r.config.RoleArn),
		RoleSessionName: aws.String("uploader_"),
	})
	if err == nil {
		// return response.Credentials, nil
		return &Credentials{
			AccessKeySecret: *response.Credentials.SecretAccessKey,
			Expiration:      (*response.Credentials.Expiration).Format(time.DateTime),
...
func (r *S3) ListObjects(ctx context.Context, opt *ListObjectOpts) (*ListObjectRes, error) {
	res := ListObjectRes{
