...
type dbModel struct {
	...
	TestTime    time.Time `json:"test_time" gdb:"ip_long;type:Unix"`               // 测试时间 使用gdb重命名和设置时间格式为秒
	TestTimeMil time.Time `json:"test_time_mil" gdb:"peer_ip_long;type:UnixMilli"` // 测试时间 使用gdb重命名和设置时间格式为毫秒
	CreateId    int64     `json:"create_id"`                                       // 创建者ID
	CreateTs    int64     `json:"create_ts"`                                       // 创建者TS
	CreateTime  time.Time `json:"create_time" gdb:"type:2006-01-02"`               // 创建时间 使用gdb设置时间格式为 年-月-日
