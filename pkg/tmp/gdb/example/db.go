...
func dbCoherent() {
	...
	gdb.NewCtx(context.Background(), "table")
	// 连贯操作函数
	gdb.New().
		// 设置操作表
		Table("table").
		...
		WhereGroup(func(gs *gdb.Db) (err error) {
			gs.Where("id = ?", 1).WhereOr("id = ?", 1)
			return nil
		}).
		// 条件组 OR (id = ? OR id = ?) 带括号包裹
		WhereGroupOr(func(gs *gdb.Db) (err error) {
			gs.Where("id = ?", 1).WhereOr("id = ?", 1)
			return nil
		}).
		// 查询字段
		...
		ConvFieldsType(func(key string, val any, m *gdb.ReadOnlyMap) any {
			// m 为当前结果值
			switch key {
			case "id", "ts":
				intVal, _ := conv.ToInt(val) // id 和 ts 返回数组类型给前端 避免出现 id:"1" 的情况
				return intVal
			}
			// 保持原样
			return val
...
func dbEdn() {
	...
	gdb.NewCtx(context.Background(), "table")
	// 查询单条数据到结构体或map自动判断
	err = gdb.New().Scan(&res).Error
	// 查询多条数据到结构体或map自动判断
	err = gdb.New().Scan(&resList).Error
	// 查询单条数据到结构体或map
	err = gdb.New().One(&res).Error
	// 查询多条数据到结构体或map
	err = gdb.New().Select(&resList).Error
	// 查询总数
	err = gdb.New().Count(&total).Error
	// 更新某个字段
	err = gdb.New().Update("name", "new name").Error
	// 更新多个字段 支持结构体
	err = gdb.New().Updates(map[string]any{
		"name": "new name",
		"age":  18,
	}).Error
	// 连表更新
	err = gdb.New("table t1").
		LeftJoin("t t2", "t1.id = t2.id").
		Updates(map[string]gdb.RawBody{
			"t1.name": "t2.name",
			"t1.age":  "t2.age",
		}).Error
	// 更新多个字段 忽略冲突 支持结构体
	err = gdb.New().UpdateIgnore(map[string]any{
		"name": "new name",
		"age":  18,
	}).Error
	// 存在更新，否则新增 data = map[string]any 或者 Struct
	err = gdb.New().Save(map[string]any{
		"name": "new name",
		"age":  18,
	}).Error
	// 存在更新，否则新增 批量  data = []map[string]any 或者 []Struct blockSize 批量大小 默认100
	err = gdb.New().SaveInBatches([]map[string]any{
		{
			"name": "new name",
			"age":  18,
		},
	...
	// 删除数据
	err = gdb.New().Delete().Error
	// 连表删除
	err = gdb.New().Table("table t1").
		LeftJoin("tb t2", "t1.id = t2.id").
		Where("t2.id IS NULL").
		Delete("t1").Error
	// 新增数据
	err = gdb.New().Create(map[string]any{
		"name": "new name",
		"age":  18,
	}).Error
	// 从查询新增数据 INSERT INTO table (name,age) SELECT name,age FROM table
	err = gdb.New().Field("name,age").Create(gdb.New("table").Field("name,age")).Error
	// 批量新增数据 data = []map[string]any 或者 []Struct blockSize 批量大小 默认100
	err = gdb.New().CreateInBatches([]map[string]any{
		{
			"name": "new name",
			"age":  18,
		},
	...
	res, err = gdb.New().Map()
	// 查询直接返回Map列表
	resList, err = gdb.New().Maps()
	// 事务查询
	err = gdb.New().Transaction(func(tx *gdb.DbTx) error {
		err = gdb.New("table").Select("").Error
		err = gdb.New("table").Delete().Error
...
// 流式查询
func stream() {
	var err = gdb.New("tb").Where("id > 0").Stream(gdb.StreamCallback(func(t map[string]any) (next bool, err error) {
		fmt.Println(t)
		return true, nil
	})).Error
	fmt.Println(err)
