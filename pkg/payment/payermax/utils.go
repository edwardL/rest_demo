...
func GetRsaSign(body []byte, privateKey *rsa.PrivateKey) (sign string, err error) {
	h := sha256.New()
	hashs := crypto.SHA256
...
func VerifySign(signData []byte, sign string, publicKey *rsa.PublicKey) (err error) {
	signBytes, err := base64.StdEncoding.DecodeString(sign)
	if err != nil {
		return fmt.Errorf("base64 decode sign error: %w", err)
	}
	hashs := crypto.SHA256
	h := hashs.New()
	h.Write(signData)
